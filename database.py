...
def get_user(telegram_id) -> dict | None:
    ...
    # چون کیف‌پول/بلاک‌بودن جای حساسیه، فقط برای مدت خیلی کوتاه (۲ ثانیه) کش
    # می‌کنیم؛ یعنی این تکرارهای پشت‌سرهم داخل همون یک آپدیت دیگه به دیتابیس
    # نمی‌رن، ولی بعد از چند ثانیه (مثلاً بعد از شارژ کیف‌پول) همیشه مقدار
    ...
    if value is not None:
        cache.set(cache_key, value, ttl=2)
    return value
