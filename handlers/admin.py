...
import panels
import ui_editor
from permissions import permission_for_callback, permission_for_message
from resilience import panel_health_check
...
def _permission_for_message_text(text: str | None) -> str | None:
    return permission_for_message(text)

def _permission_for_callback(data: str | None) -> str | None:
    return permission_for_callback(data)
...
class AdminPermissionMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        if cb and (cb=="admin_manage_admins" or cb.startswith("subadm")):
            await event.answer("⛔ فقط ادمین اصلی دسترسی دارد.", show_alert=True); return
        perm=_permission_for_callback(cb) if cb is not None else _permission_for_message_text(getattr(event,"text",None))
        if perm=="receipts" and db.sub_admin_has_permission(str(user.id),"requests"):
            return await handler(event,data)
        if perm and not db.sub_admin_has_permission(str(user.id),perm):
            if cb is not None: await event.answer("⛔ شما به این قابلیت دسترسی ندارید.",show_alert=True)
            else: await event.answer("⛔ شما به این قابلیت دسترسی ندارید.")
            return
        if perm:
            try:
                db.audit_event("admin", "permission.route", "permission", perm, f"callback={cb or getattr(event,'text', '')}", actor_id=str(user.id), actor_name=user.full_name)
            except Exception:
...
def _current_admin_permissions(user_id: int):
...
async def admin_entry(message: types.Message):
    ...
    await message.answer("منوی مدیریت پایین صفحه هم فعال است ✅", reply_markup=_admin_reply_kb_for(message.from_user.id))


async def _show_admin_home(callback: types.CallbackQuery, state: FSMContext):
    """برگشت مطمئن به صفحه اصلی ادمین.

    این مسیر عمداً مستقل از نوع پیام است: دکمه «بازگشت» ممکن است روی پیام متنی،
    عکس/کپشن یا پیامی باشد که دیگر قابل edit نیست. در هر سه حالت باید کاربر
    حتماً به پنل اصلی برسد و callback هم از حالت loading خارج شود.
    """
    ...
    await state.clear()
    # اول callback را پاسخ می‌دهیم تا حتی اگر عملیات ویرایش پیام شکست خورد،
    # دکمه در Telegram در حالت لودینگ باقی نماند.
    try:
        await callback.answer()
    except Exception:
        pass

    kb = _admin_panel_kb_for(user_id)
    message = callback.message
    if message is None:
        return

    # پیام‌های عکس/ویدئو/سایر media با edit_text قابل ویرایش نیستند.
    try:
        if getattr(message, "photo", None) or getattr(message, "video", None) or getattr(message, "document", None) or getattr(message, "animation", None):
            await message.edit_caption(caption="👨‍💻 پنل مدیریت:", reply_markup=kb)
        else:
            await message.edit_text("👨‍💻 پنل مدیریت:", reply_markup=kb)
        return
    except Exception as exc:
        logger.warning("admin_back edit failed; falling back to a new message: %s", exc)

    # اگر پیام قابل edit نبود، پنل را به‌صورت پیام جدید نمایش می‌دهیم.
    try:
        await message.answer("👨‍💻 پنل مدیریت:", reply_markup=kb)
    except Exception:
        logger.exception("admin_back fallback answer failed")


@router.callback_query(F.data == "admin_back")
async def admin_back(callback: types.CallbackQuery, state: FSMContext):
    await _show_admin_home(callback, state)
...
@router.message(F.text.in_({"🦖 لاگ خطاها (Sentry)", "🦖 لاگ خطاها و Audit"}))
...
# ---------------------------------------------------------------------------
# Aliasهای Callback برای دکمه‌های منوی بالای ادمین
# ---------------------------------------------------------------------------
...
async def admin_botinfo_alias(callback: types.CallbackQuery, state: FSMContext):
    await admin_botinfo_open(callback, state)

@router.callback_query(F.data == "admin_free_test_settings")
async def admin_free_test_settings_callback(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    await state.set_state(AdminStates.waiting_free_test_settings)
    await callback.message.edit_text(_free_test_settings_text(), reply_markup=admin_back_button())
    await callback.answer()

@router.callback_query(F.data == "admin_custom_build_settings")
async def admin_custom_build_settings_callback(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    await state.set_state(AdminStates.waiting_custom_build_settings)
    await callback.message.edit_text(_custom_build_settings_text(), reply_markup=admin_back_button())
    await callback.answer()
...
# ---------------------------------------------------------------------------
# ورودی‌های منوی پایین ادمین
# ---------------------------------------------------------------------------
# این‌ها عمداً به‌صورت message handler هم تعریف شده‌اند. دکمه‌های منوی پایین
# ReplyKeyboard هستند و callback handlerها به‌تنهایی نمی‌توانند به آن‌ها پاسخ بدهند.

@router.message(F.text.in_({"👥 لیست کاربران", "👥 کاربران و سرویس‌ها"}))
...
@router.message(F.text.in_({"🔍 جستجوی حرفه‌ای", "🔍 جستجوی کاربر", "🔎 جستجوی کاربر"}))
async def menu_admin_search(message: types.Message, state: FSMContext):
    if not _is_admin(message.from_user.id):
        return
    await state.clear()
    await message.answer(
        "🔍 آیدی عددی یا کد دعوت کاربر را ارسال کنید:",
        reply_markup=admin_back_button(),
    )
    await state.set_state(AdminStates.waiting_search_user)


@router.message(F.text == "📝 ویرایش متن و دکمه‌ها")
async def menu_admin_text_editor(message: types.Message, state: FSMContext):
    if not _is_admin(message.from_user.id):
        return
    await state.clear()
    await message.answer(
        "📝 ویرایشگر ربات\n\nچه چیزی را می‌خواهید تغییر دهید؟",
        reply_markup=ui_editor.editor_mode_keyboard(),
    )


@router.message(F.text == "🎬 استیکرهای منو")
async def menu_admin_stickers(message: types.Message, state: FSMContext):
    if not _is_admin(message.from_user.id):
        return
    await state.clear()
    sections = []
    for key in _STICKER_SECTION_ORDER:
        ...
        sections.append({
            "key": key,
            "label": STICKER_SECTION_LABELS.get(key, key),
            "status_emoji": emoji,
        })
    text = (
...
async def _log_fulfilled_order(
    ...
    # منبع حقیقت نام سرویس و مشخصات، خود پنل است؛ مخصوصاً برای شاهراه که نام ایجادشده
    # ممکن است با نام پلن فروشگاه فرق داشته باشد.
    if target_config_id or service_id:
        cfg = db.get_config_by_id(target_config_id) if target_config_id else None
        if cfg and cfg.get("panel_id") and cfg.get("service_id"):
            panel = db.get_vpn_panel(cfg["panel_id"])
            if panel:
                try:
                    ok_live, live, _ = await panels.get_service_snapshot(panel, cfg["service_id"])
                    if ok_live and live:
                        service_name = live.get("username") or service_name
                        if live.get("total") is not None:
                            package_text = f"{round(live['total'] / (1024**3), 2):g} گیگابایت" + (f" | {package_text.split('|')[-1].strip()}" if package_text and '|' in package_text else "")
                        if live.get("expire"):
                            try:
                                expiry_text = datetime.fromtimestamp(int(live["expire"])).strftime("%Y-%m-%d")
                            except Exception:
                                pass
                except Exception:
                    logger.exception("دریافت نام واقعی سرویس از پنل برای لاگ اعتماد ناموفق بود")
...
async def _finalize_send(message: types.Message, state: FSMContext):
    ...
    # متن تحویل کانفیگ هم از ویرایشگر متن استفاده می‌کند و مقادیر داینامیک را
    # با placeholderهای امن نگه می‌دارد تا ادمین بتواند ظاهر متن را تغییر دهد
    # بدون اینکه نام/حجم/مدت/لینک واقعی از بین برود.
    import ui_editor
    delivery_fallback = (
    ...
    caption, caption_entities = ui_editor.render_template(
        "config_delivery",
        {
            "name": name,
            "location": "مولتی لوکیشن+تانل",
            "days": days_text,
            "volume": volume_text,
            "users": "نامحدود",
            "sub_link": sub_link,
...
async def admin_text_editor(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    await callback.message.edit_text(
...
async def admin_ui_mode(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    mode = callback.data.split(":", 1)[1]
    if mode == "text":
        title = "✏️ ویرایش متن ربات"
    elif mode == "buttons":
        title = "🔘 ویرایش دکمه‌های ربات"
    else:
        await callback.answer("❌ گزینه نامعتبر است.", show_alert=True)
        return
    await callback.message.edit_text(
        f"{title}\n\nدسته موردنظر را انتخاب کنید:",
        reply_markup=ui_editor.categories_keyboard(mode),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_cat:"))
async def admin_ui_category(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    parts = callback.data.split(":", 2)
    if len(parts) != 3:
        await callback.answer("❌ مسیر نامعتبر است.", show_alert=True)
        return
    mode, cat = parts[1], parts[2]
    await callback.message.edit_text(
        f"📝 بخش: {ui_editor.category_label(cat)}\n\nصفحه موردنظر را انتخاب کنید:",
        reply_markup=ui_editor.screens_keyboard(cat, mode),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_auto_groups:"))
async def admin_ui_auto_groups(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    try:
        ...
        page = 0
    await callback.message.edit_text(
        "🧩 تمام پیام‌ها و اعلان‌ها\n\nپیام‌ها بر اساس بخشی که در ربات ازشون استفاده می‌شه دسته‌بندی شدن. "
        "دسته‌ی موردنظر رو انتخاب کن:",
        reply_markup=ui_editor.auto_source_groups_keyboard(page),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_auto_group:"))
async def admin_ui_auto_group(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    parts = callback.data.split(":", 2)
    ...
    try:
        ...
        page = 0
    label = ui_editor._source_group_label(source_file)
    await callback.message.edit_text(
        f"{label}\n\nصفحه {page + 1} — متن موردنظر را انتخاب کنید:",
        reply_markup=ui_editor.auto_messages_keyboard(source_file, page),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_buttons_page:"))
async def admin_ui_buttons_page(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    try:
        ...
        page = 0
    await callback.message.edit_text(
        f"🔘 تمام دکمه‌های قابل ویرایش\n\nصفحه {page + 1} — صفحه موردنظر را انتخاب کنید:",
        reply_markup=ui_editor.all_buttons_keyboard(page),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_auto:"))
async def admin_ui_auto_preview(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    key = callback.data.split(":", 1)[1]
    item = ui_editor.get_auto_entry(key)
    if not item:
        await callback.answer("❌ پیام پیدا نشد.", show_alert=True)
        return
    preview = ui_editor.get_text(key, item["template"])
    text = f"📝 پیش‌نمایش پیام\n\n{preview}\n\n📍 منبع: {item['source']}"
    await callback.message.edit_text(text[:3900], reply_markup=ui_editor.auto_preview_keyboard(key))
    await callback.answer()


@router.callback_query(F.data.startswith("ui_edit_auto:"))
async def admin_ui_edit_auto(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    key = callback.data.split(":", 1)[1]
    ...
    await state.set_state(AdminStates.waiting_ui_text)
    await callback.message.answer(
        "✏️ متن جدید این پیام را ارسال کنید.\n\n"
        "اگر متن داینامیک است، Placeholderهای {{AUTO1}}، {{AUTO2}} و ... را در متن جدید نگه دارید "
        "تا مقادیر متغیر قبلی در زمان ارسال جایگزین شوند.\n\n"
        f"پیش‌نمایش فعلی:\n{ui_editor.get_text(key, item['template'])[:3000]}"
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_screen:"))
async def admin_ui_screen(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    parts = callback.data.split(":", 2)
    ...
    screen = ui_editor.get_screen(key)
    if not screen:
    ...
    if mode == "text":
        await callback.message.edit_text(
            ui_editor.preview_text(key),
            reply_markup=ui_editor.text_screen_keyboard(key),
        )
    else:
        await callback.message.edit_text(
            f"🔘 ویرایش دکمه‌های «{screen['label']}»\n\n"
            "از دکمه‌های ⬆️ و ⬇️ برای ترتیب، از 👁 برای نمایش/مخفی و از 📐 برای تعداد دکمه در هر ردیف استفاده کنید.",
            reply_markup=ui_editor.button_screen_keyboard(key),
        )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_screen_textpage:"))
async def admin_ui_screen_text_page(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    parts = callback.data.split(":")
    if len(parts) != 3:
        await callback.answer("❌ مسیر نامعتبر است.", show_alert=True); return
    key = parts[1]
    try: page = max(0, int(parts[2]))
    except ValueError: page = 0
    if not ui_editor.get_screen(key):
        await callback.answer("❌ صفحه پیدا نشد.", show_alert=True); return
    await callback.message.edit_text(
        ui_editor.preview_text(key, page),
        reply_markup=ui_editor.text_screen_keyboard(key, page),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_edit_text:"))
async def admin_ui_edit_text(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    key = callback.data.split(":", 1)[1]
    ...
    await state.set_state(AdminStates.waiting_ui_text)
    await callback.message.answer(
        "✏️ متن جدید را در یک پیام ارسال کنید.\n\n"
        "برای Premium/Custom Emoji، همان ایموجی پرمیوم را مستقیماً از تلگرام داخل پیام بفرستید؛ "
        "رنگ و شناسه آن حفظ می‌شود."
    )
    await callback.answer()


@router.message(AdminStates.waiting_ui_text)
async def admin_ui_text_save(message: types.Message, state: FSMContext):
    data = await state.get_data()
    key = data.get("ui_key")
    if not key:
        await state.clear()
    ...
    # fix/ایمنی: اگر این متن داینامیک باشه (شامل مقادیری مثل موجودی، قیمت،
    # نام کاربر و ...) و ادمین موقع ویرایش یکی از Placeholder های {{AUTOn}}
    # رو حذف کنه، اون مقدار برای همیشه از پیام حذف می‌شه بدون هیچ خطایی —
    # مثلاً «موجودی: {{AUTO1}} تومان» تبدیل بشه به «موجودی:» و دیگه هیچ‌وقت
    # عدد نشون داده نشه. قبل از ذخیره، حضور همه‌ی Placeholder های لازم رو
    # چک می‌کنیم.
    auto_item = ui_editor.get_auto_entry(key)
    if auto_item and auto_item.get("dynamic"):
        required = set(re.findall(r"\{\{AUTO\d+\}\}", auto_item["template"]))
        missing = required - set(re.findall(r"\{\{AUTO\d+\}\}", text))
        if missing:
            missing_list = "، ".join(sorted(missing))
            await message.answer(
                f"❌ این متن داینامیک است و باید همه‌ی Placeholder های زیر رو داخل متن جدید نگه داری، "
                f"وگرنه اون مقدار (مثلاً عدد یا نام) دیگه توی پیام نشون داده نمی‌شه:\n\n{missing_list}\n\n"
                "متن جدید را با نگه‌داشتن همه‌ی این‌ها دوباره بفرست:"
            )
            return

    ui_editor.set_text(key, text, entities)
    await state.clear()
    back_markup = ui_editor.text_screen_keyboard(key) if not (auto_item) else None
    if back_markup is None:
        source_file = (auto_item.get("source") or "").split(":")[0] if auto_item else ""
        back_markup = ui_editor.auto_messages_keyboard(source_file, 0) if source_file else ui_editor.auto_source_groups_keyboard(0)
    await message.answer("✅ متن ذخیره شد.", reply_markup=back_markup)


@router.callback_query(F.data.startswith("ui_button_page:"))
async def admin_ui_button_page(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    parts = callback.data.split(":")
    if len(parts) != 3:
    ...
    await callback.message.edit_text(
        f"🔘 ویرایش دکمه‌های «{screen['label']}»\n\nصفحه {page + 1}",
        reply_markup=ui_editor.button_screen_keyboard(key, page),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_button:"))
async def admin_ui_button(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    _, key, cb = callback.data.split(":", 2)
    await state.update_data(ui_key=key, ui_cb=cb)
    await state.set_state(AdminStates.waiting_ui_button_text)
    meta = ui_editor.get_button_meta(key, cb)
    status = "مخفی" if meta.get("hidden") else "نمایش"
    await callback.message.answer(
        f"🔘 ویرایش دکمه\n\nوضعیت فعلی: {status}\n"
        "نام جدید دکمه را ارسال کنید. اگر Premium/Custom Emoji می‌خواهید، همان ایموجی پرمیوم را "
        "داخل پیام بفرستید؛ اولین Custom Emoji به‌عنوان آیکن دکمه ذخیره می‌شود."
    )
    await callback.answer()


@router.message(AdminStates.waiting_ui_button_text)
async def admin_ui_button_save(message: types.Message, state: FSMContext):
    data = await state.get_data()
    key, cb = data.get("ui_key"), data.get("ui_cb")
    ...
    for e in (source_entities or []):
        if getattr(e, "custom_emoji_id", None):
            ...
            break
    text = message.text if message.text is not None else (message.caption or "")
    ui_editor.set_button(key, cb, text, custom_emoji_id=custom_emoji_id)
    await state.clear()
    await message.answer("✅ نام دکمه و در صورت وجود Premium Emoji ذخیره شد.", reply_markup=ui_editor.button_screen_keyboard(key))


@router.callback_query(F.data.startswith("ui_toggle:"))
async def admin_ui_toggle(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    _, key, cb = callback.data.split(":", 2)
    ui_editor.toggle_button(key, cb)
    await callback.message.edit_reply_markup(reply_markup=ui_editor.button_screen_keyboard(key))
    await callback.answer("✅ وضعیت دکمه تغییر کرد.")


@router.callback_query(F.data.startswith("ui_move:"))
async def admin_ui_move(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    _, key, cb, direction = callback.data.split(":", 3)
    ui_editor.move_button(key, cb, direction)
    await callback.message.edit_reply_markup(reply_markup=ui_editor.button_screen_keyboard(key))
    await callback.answer("✅ ترتیب ذخیره شد.")


@router.callback_query(F.data.startswith("ui_layout:"))
async def admin_ui_layout(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    parts = callback.data.split(":")
    key = parts[1]
    mode = parts[2] if len(parts) > 2 else "cycle"
    requested = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 1

    # تنظیم چیدمان واقعی: 1/2/3/4 دکمه در هر ردیف.
    # تقسیم‌پذیری کامل لازم نیست؛ مثلاً 7 دکمه با 2 ستون => 2+2+2+1.
    # «set» مسیر اصلی و قطعی Editor است؛ «cycle» و «inline» فقط برای
    # سازگاری با Callbackهای نسخه‌های قبلی نگه داشته شده‌اند.
    if mode in ("set", "cycle"):
        columns = min(4, max(1, requested))
        target_mode = "vertical" if columns == 1 else "inline"
    elif mode == "inline":
        current = ui_editor.get_layout(key)
        columns = current["columns"] + 1 if current["mode"] == "inline" else 2
        if columns > 4:
            columns = 1
        target_mode = "vertical" if columns == 1 else "inline"
    else:
    ...
    ui_editor.set_layout(key, target_mode, columns)
    # اطمینان از اینکه تنظیم جدید واقعاً در DB خوانده شده و UI همان مقدار را
    # نمایش می‌دهد؛ این جلوی حالتی را می‌گیرد که فقط متن دکمه عوض شود ولی
    # چیدمان ذخیره نشده باشد.
    saved = ui_editor.get_layout(key)
    saved_cols = int(saved.get("columns") or 1)
    await callback.message.edit_reply_markup(reply_markup=ui_editor.button_screen_keyboard(key))
    await callback.answer(f"✅ چیدمان ذخیره شد: {saved_cols} دکمه در هر ردیف")


# ---------------------------------------------------------------------------
# ➕ دکمه‌های سفارشی (افزودن دکمه‌ی دلخواه با کالبک/لینک/اپ‌لینک به هر صفحه)
# ---------------------------------------------------------------------------

@router.callback_query(F.data.startswith("ui_cbtn_list:"))
async def admin_cbtn_list(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    key = callback.data.split(":", 1)[1]
    await state.clear()
    await callback.message.edit_text(
        f"➕ دکمه‌های سفارشی صفحه‌ی «{ui_editor.get_screen(key)['label'] if ui_editor.get_screen(key) else key}»\n\n"
        "این دکمه‌ها علاوه بر دکمه‌های پیش‌فرض این صفحه، پایین همان صفحه اضافه می‌شوند.\n"
        "روی هرکدوم بزن برای جابه‌جایی/حذف، یا از پایین یکی جدید اضافه کن:",
        reply_markup=ui_editor.custom_buttons_keyboard(key),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_cbtn_open:"))
async def admin_cbtn_open(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    btn = ui_editor.get_custom_button(int(callback.data.split(":", 1)[1]))
    if not btn:
        await callback.answer("❌ این دکمه دیگر وجود ندارد.", show_alert=True)
        return
    kind_label = {"callback": "🔗 داخلی", "url": "🌐 لینک وب", "webapp": "📲 اپ‌لینک"}.get(btn["action_type"], btn["action_type"])
    await callback.answer(
        f"متن: {btn['text']}\nنوع: {kind_label}\nمقدار: {btn['action_value']}"[:200],
        show_alert=True,
    )


@router.callback_query(F.data.startswith("ui_cbtn_move:"))
async def admin_cbtn_move(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    _, btn_id, direction = callback.data.split(":")
    btn = ui_editor.get_custom_button(int(btn_id))
    if not btn:
        await callback.answer("❌ این دکمه دیگر وجود ندارد.", show_alert=True)
        return
    ui_editor.move_custom_button(int(btn_id), direction)
    await callback.message.edit_reply_markup(reply_markup=ui_editor.custom_buttons_keyboard(btn["screen_key"]))
    await callback.answer("✅ جابه‌جا شد.")


@router.callback_query(F.data.startswith("ui_cbtn_del:"))
async def admin_cbtn_del(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    btn = ui_editor.get_custom_button(int(callback.data.split(":", 1)[1]))
    if not btn:
        await callback.answer("❌ این دکمه دیگر وجود ندارد.", show_alert=True)
        return
    ui_editor.delete_custom_button(btn["id"])
    await callback.message.edit_reply_markup(reply_markup=ui_editor.custom_buttons_keyboard(btn["screen_key"]))
    await callback.answer("🗑 حذف شد.")


@router.callback_query(F.data.startswith("ui_cbtn_add:"))
async def admin_cbtn_add_start(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    key = callback.data.split(":", 1)[1]
    await state.update_data(cbtn_screen=key)
    await state.set_state(AdminStates.waiting_custom_button_text)
    await callback.message.edit_text(
        "✏️ متن نمایشی دکمه‌ی جدید را بفرست (همانی که روی خود دکمه دیده می‌شود):",
        reply_markup=admin_back_button(),
    )
    await callback.answer()


@router.message(AdminStates.waiting_custom_button_text)
async def admin_cbtn_add_text(message: types.Message, state: FSMContext):
    if not _is_admin(message.from_user.id):
        return
    text = (message.text or "").strip()
    if not text:
        await message.answer("❌ متن نمی‌تواند خالی باشد؛ دوباره بفرست:")
        return
    data = await state.get_data()
    key = data.get("cbtn_screen")
    if not key:
        await state.clear()
        return
    await state.update_data(cbtn_text=text)
    await state.set_state(None)
    await message.answer("🎨 رنگ/استایل این دکمه را انتخاب کن:", reply_markup=ui_editor.custom_button_style_keyboard(key))


@router.callback_query(F.data.startswith("ui_cbtn_style:"))
async def admin_cbtn_pick_style(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    _, key, style = callback.data.split(":")
    await state.update_data(cbtn_style=style)
    await callback.message.edit_text(
        "⚙️ این دکمه وقتی زده شد چه کاری انجام بدهد؟",
        reply_markup=ui_editor.custom_button_action_type_keyboard(key),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("ui_cbtn_type:"))
async def admin_cbtn_pick_type(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    _, key, kind = callback.data.split(":")
    if kind == "callback_quick":
        await callback.message.edit_text(
            "🔗 دکمه به کدام بخش از ربات وصل شود؟",
            reply_markup=ui_editor.quick_destination_keyboard(key),
        )
        await callback.answer()
        return
    action_type = {"callback_manual": "callback", "url": "url", "webapp": "webapp"}.get(kind)
    if not action_type:
        await callback.answer("❌ نوع نامعتبر.", show_alert=True)
        return
    await state.update_data(cbtn_action_type=action_type)
    await state.set_state(AdminStates.waiting_custom_button_value)
    prompts = {
        "callback": "✍️ callback_data داخلی را دقیقاً وارد کن (پیشرفته؛ برای کاربران عادی از گزینه‌ی «وصل به یکی از صفحات» استفاده کن):",
        "url": "🌐 لینک کامل را بفرست (باید با http:// یا https:// شروع شود):",
        "webapp": "📲 آدرس Web App را بفرست (باید https:// باشد):",
    }
    await callback.message.edit_text(prompts[action_type], reply_markup=admin_back_button())
    await callback.answer()


@router.callback_query(F.data.startswith("ui_cbtn_dest:"))
async def admin_cbtn_pick_dest(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    _, key, dest_cb = callback.data.split(":", 2)
    data = await state.get_data()
    text = data.get("cbtn_text")
    style = data.get("cbtn_style", "primary")
    if not text:
        await callback.answer("❌ ابتدا از اول شروع کن.", show_alert=True)
        return
    ui_editor.add_custom_button(key, text, style, "callback", dest_cb)
    await state.clear()
    await callback.message.edit_text("✅ دکمه اضافه شد.", reply_markup=ui_editor.custom_buttons_keyboard(key))
    await callback.answer()


@router.message(AdminStates.waiting_custom_button_value)
async def admin_cbtn_add_value(message: types.Message, state: FSMContext):
    if not _is_admin(message.from_user.id):
        return
    ...
    if not (key and text and action_type):
        await state.clear()
        return
    if action_type in ("url", "webapp") and not value.lower().startswith(("http://", "https://")):
        await message.answer("❌ لینک باید با http:// یا https:// شروع شود؛ دوباره بفرست:")
        return
    if action_type == "callback" and not value:
        await message.answer("❌ callback_data نمی‌تواند خالی باشد؛ دوباره بفرست:")
        return
    ui_editor.add_custom_button(key, text, style, action_type, value)
    await state.clear()
    await message.answer("✅ دکمه اضافه شد.", reply_markup=ui_editor.custom_buttons_keyboard(key))
...
async def admin_manage_admins_message(message: types.Message):
...
async def subadm_add_start(callback: types.CallbackQuery, state: FSMContext):
    if not _is_main_admin(callback.from_user.id): return
    await state.set_state(AdminStates.waiting_sub_admin_id)
    await callback.message.edit_text("🆔 آیدی عددی ادمین فرعی را ارسال کنید (اختیاری: نام | آیدی):", reply_markup=admin_back_button())
    await callback.answer()

@router.message(AdminStates.waiting_sub_admin_id)
async def subadm_add_apply(message: types.Message, state: FSMContext):
    if not _is_main_admin(message.from_user.id): return
    raw=(message.text or "").strip(); name=""; tid=raw
    if "|" in raw: name,tid=[x.strip() for x in raw.split("|",1)]
    if not tid.isdigit(): await message.answer("❌ آیدی معتبر نیست."); return
    db.add_sub_admin(tid,name,[])
    await state.clear()
    await message.answer("✅ ادمین اضافه شد؛ حالا دسترسی‌ها را انتخاب کنید:", reply_markup=admin_permissions_keyboard(tid,[]))

@router.callback_query(F.data.startswith("subadm_"))
async def subadm_open(callback: types.CallbackQuery):
    if not _is_main_admin(callback.from_user.id): return
    tid=callback.data.replace("subadm_",""); adm=db.get_sub_admin(tid)
    if not adm: await callback.answer("❌ ادمین پیدا نشد.",show_alert=True); return
    await callback.message.edit_text(f"👤 ادمین: {adm.get('name') or tid}\n🆔 {tid}\n\nدسترسی‌ها:", reply_markup=admin_permissions_keyboard(tid,adm.get("permissions") or [])); await callback.answer()

@router.callback_query(F.data.startswith("subadmperm_"))
async def subadm_perm_toggle(callback: types.CallbackQuery):
    if not _is_main_admin(callback.from_user.id): return
    raw=callback.data.replace("subadmperm_",""); tid,perm=raw.split(":",1); adm=db.get_sub_admin(tid)
    if not adm: await callback.answer("❌ ادمین پیدا نشد.",show_alert=True); return
    perms=set(adm.get("permissions") or [])
    if perm in perms: perms.remove(perm)
    else: perms.add(perm)
    db.update_sub_admin_permissions(tid,sorted(perms)); db.log_admin_action(str(callback.from_user.id),callback.from_user.full_name,"تغییر دسترسی ادمین",tid,perm)
    await callback.message.edit_reply_markup(reply_markup=admin_permissions_keyboard(tid,sorted(perms))); await callback.answer("✅ ذخیره شد")

@router.callback_query(F.data.startswith("subadmdel_"))
async def subadm_delete(callback: types.CallbackQuery):
    if not _is_main_admin(callback.from_user.id): return
    tid=callback.data.replace("subadmdel_",""); db.delete_sub_admin(tid); db.log_admin_action(str(callback.from_user.id),callback.from_user.full_name,"حذف ادمین فرعی",tid)
    await callback.message.edit_text("✅ ادمین حذف شد.", reply_markup=admin_manage_admins_keyboard(db.get_all_sub_admins())); await callback.answer()
...
async def menu_admin_pending_receipts(message: types.Message):
    """Bottom admin keyboard entry for the same receipt queue exposed above."""
    if not _is_admin(message.from_user.id):
        return
    receipts = db.get_pending_receipts(limit=25)
    custom_receipts = db.get_pending_custom_order_receipts(limit=25)
    ...
    if total == 0:
        ...
        text = f"🧾 رسیدهای در انتظار تایید — {total} مورد\n\nروی ✅ برای تایید یا ❌ برای رد بزن 👇"
    await message.answer(text, reply_markup=admin_pending_receipts_keyboard(receipts, custom_receipts))
...
# مدیریت سرویس‌ها — همه از طریق «🔍 جستجوی حرفه‌ای»
# ---------------------------------------------------------------------------
...
async def _service_detail_text(cfg: dict) -> str:
    ...
    if panel and cfg.get("service_id"):
        try:
            if panel.get("panel_type") == "shahrah" and sub_url:
                ...
                ok, snap, _msg = await panels.get_service_snapshot(panel, cfg["service_id"])
                if ok and snap:
                    usage = {"total": snap.get("total"), "used": snap.get("used"), "upload": snap.get("upload"), "download": snap.get("download"), "expire": snap.get("expire"), "status": snap.get("status")}
                    sub_url = snap.get("link") if isinstance(snap.get("link"), str) else None
        except Exception:
            logger.exception("live panel service detail failed")
    elif cfg.get("source") == "shahrah" and sub_url:
        try:
            usage = await fetch_subscription_info(sub_url)
        except Exception:
            usage = None
    ...
    if panel and panel.get("panel_type") in ("marzban", "pasargad"):
        text += f"\n🖥 منبع زنده: {html.escape(panels.panel_label(panel))}\n"
        if sub_url:
            text += f"🔗 لینک ساب پنل:\n<code>{html.escape(str(sub_url))}</code>\n"
    else:
        text += f"\n🔗 لینک ساب فعلی:\n<code>{sub_preview_safe}</code>\n"
    if cfg.get("type", "vip") == "vip":
        text += f"\n🎫 کیوآرکد: {'ثبت شده ✅' if cfg.get('qr_file_id') else 'ثبت نشده ❌'}"
    # fix: قبلاً اینجا به یک متغیر `files` تعریف‌نشده رفرنس داده می‌شد که باعث
    # می‌شد نمایش جزئیات هر سرویس غیر VIP (مثلاً سرویس‌های «تست رایگان» با
    # type=test) با NameError کرش کنه و کل صفحه‌ی «مشاهده سرویس‌های کاربر»
    # با خطا مواجه بشه. این نوع سرویس‌ها فایل جداگانه ندارن، پس فقط رد می‌شیم.
    return text
...
async def admin_service_regenerate_link(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    try: cfg_id = int(callback.data.replace("panelrevoke_", ""))
    except ValueError:
        await callback.answer("❌ شناسه سرویس نامعتبر است.", show_alert=True); return
    cfg = db.get_config_by_id(cfg_id)
    panel = db.get_vpn_panel(cfg.get("panel_id")) if cfg and cfg.get("panel_id") else None
    if not cfg or not panel or not cfg.get("service_id"):
        await callback.answer("❌ این سرویس به پنل قابل مدیریت متصل نیست.", show_alert=True); return
    await callback.answer("⏳ در حال ایجاد لینک جدید از پنل...")
    ok, link, remote_id, _raw, msg = await panels.regenerate_sub_link(panel, str(cfg["service_id"]))
    if not ok or not link:
        await callback.message.answer(f"❌ تغییر لینک انجام نشد.\n\n{msg}"); return
    db.update_config_link(cfg_id, crypto.encrypt_config(link)); db.set_config_link_disabled(cfg_id, False)
    await _render_service_detail(callback, cfg_id)

@router.callback_query(F.data.startswith("paneldisable_"))
async def admin_service_disable(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    try: cfg_id = int(callback.data.replace("paneldisable_", ""))
    except ValueError:
        await callback.answer("❌ شناسه نامعتبر است.", show_alert=True); return
    cfg = db.get_config_by_id(cfg_id) if cfg_id else None
    panel = db.get_vpn_panel(cfg.get("panel_id")) if cfg and cfg.get("panel_id") else None
    if not cfg or not panel or not cfg.get("service_id"):
        await callback.answer("❌ سرویس به پنل متصل نیست.", show_alert=True); return
    await callback.answer("⏳ در حال غیرفعال‌سازی...")
    ok, msg = await panels.disable_service(panel, str(cfg["service_id"]))
    if not ok:
        await callback.message.answer(f"❌ غیرفعال‌سازی ناموفق بود:\n{msg}"); return
    db.set_config_link_disabled(cfg_id, True); await _render_service_detail(callback, cfg_id)

@router.callback_query(F.data.startswith("panelenable_"))
async def admin_service_enable(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    try: cfg_id = int(callback.data.replace("panelenable_", ""))
    except ValueError:
        await callback.answer("❌ شناسه نامعتبر است.", show_alert=True); return
    cfg = db.get_config_by_id(cfg_id) if cfg_id else None
    panel = db.get_vpn_panel(cfg.get("panel_id")) if cfg and cfg.get("panel_id") else None
    if not cfg or not panel or not cfg.get("service_id"):
        await callback.answer("❌ سرویس به پنل متصل نیست.", show_alert=True); return
    await callback.answer("⏳ در حال فعال‌سازی...")
    ok, msg = await panels.enable_service(panel, str(cfg["service_id"]))
    if not ok:
        await callback.message.answer(f"❌ فعال‌سازی ناموفق بود:\n{msg}"); return
    db.set_config_link_disabled(cfg_id, False); await _render_service_detail(callback, cfg_id)

@router.callback_query(F.data.startswith("panelrenew_"))
async def admin_service_renew(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True); return
    try: cfg_id = int(callback.data.replace("panelrenew_", ""))
    except ValueError:
        await callback.answer("❌ شناسه نامعتبر است.", show_alert=True); return
    cfg = db.get_config_by_id(cfg_id) if cfg_id else None
    panel = db.get_vpn_panel(cfg.get("panel_id")) if cfg and cfg.get("panel_id") else None
    ...
    remote_ref = None
    try:
        for pk, plan in (db.get_all_plans() or {}).items():
            if str(plan.get("name", "")) == str(cfg.get("plan", "")) and plan.get("id") is not None:
                mapping = db.get_panel_plan_map_with_panel("vip_plan", int(plan["id"]))
                if mapping and int(mapping.get("panel_id") or 0) == int(panel.get("id") or 0):
                    remote_ref = mapping.get("remote_ref")
                break
    except Exception:
        remote_ref = None
    await callback.answer("⏳ در حال تمدید سرویس از پنل...")
    if remote_ref is not None:
        ok, link, remote_id, _raw, msg = await panels.renew_service(panel, str(cfg["service_id"]), remote_ref=remote_ref)
    else:
        ok_snap, snap, snap_msg = await panels.get_service_snapshot(panel, str(cfg["service_id"]))
        if not ok_snap or not snap:
            await callback.message.answer(f"❌ وضعیت سرویس برای تمدید خوانده نشد:\n{snap_msg}"); return
        try:
            total=int(snap.get("total") or 0); used=int(snap.get("used") or 0)
            remaining_gb=max(1,int((max(0,total-used)/(1024**3))+0.999)) if total else None
        except Exception: remaining_gb=None
        try:
            rem=days_remaining(snap.get("expire")); remaining_days=max(1,int(rem)) if rem is not None else None
        except Exception: remaining_days=None
        if panel.get("panel_type") == "shahrah":
            await callback.message.answer("❌ نگاشت بسته‌ی این سرویس برای شاهراه پیدا نشد؛ ابتدا نگاشت پلن را اصلاح کنید."); return
        if remaining_gb is None and remaining_days is None:
            await callback.message.answer("❌ حجم/زمان قابل تشخیص نیست؛ ابتدا نگاشت پلن را اصلاح کنید."); return
        ok, link, remote_id, _raw, msg = await panels.renew_service(panel, str(cfg["service_id"]), remote_ref=None, volume_gb=remaining_gb, days=remaining_days)
    if not ok:
        await callback.message.answer(f"❌ تمدید ناموفق بود:\n{msg}"); return
    if link: db.update_config_link(cfg_id, crypto.encrypt_config(link))
...
async def admin_service_purge_confirm(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        "⚠️ این کار غیرقابل بازگشته و کل اطلاعات این سرویس برای همیشه پاک می‌شه.\n\nمطمئنی؟",
...
async def admin_vip_plan_detail(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"👥 سقف دستگاه: {plan.get('user_limit') or 'نامحدود'}\n"
...
router.callback_query(F.data.startswith("vipplanlimit_"))(
    _vip_plan_edit_starter(AdminStates.waiting_vip_plan_edit_user_limit, "👥 سقف دستگاه/HWID را از 0 تا 10 ارسال کنید (0 = نامحدود):", "vipplanlimit_")
)
...
async def admin_vip_plan_edit_user_limit_apply(message: types.Message, state: FSMContext):
    data = await state.get_data()
    plan_key = data.get("edit_vip_plan_key")
    if not plan_key or not message.text or not message.text.isdigit():
        await message.answer("❌ فقط عدد ۰ تا ۱۰ ارسال کنید:")
        return
    limit = int(message.text)
    if not 0 <= limit <= 10:
        await message.answer("❌ مقدار باید بین ۰ تا ۱۰ باشد.")
        return
    db.update_vip_plan(plan_key, user_limit=limit)
    await _after_vip_plan_edit(message, state, plan_key, f"✅ سقف دستگاه/HWID بروزرسانی شد: {limit if limit else 'نامحدود'}")
...
async def admin_audit_logs(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id): return
    rows=db.get_recent_audit_logs(40)
    if not rows:
        text="🧾 Audit Log\n\nهنوز رویدادی ثبت نشده است."
    else:
        lines=["🧾 <b>Audit Log — 40 رویداد اخیر</b>"]
        for r in rows:
            actor=r.get("actor_name") or r.get("actor_id") or r.get("actor_type") or "system"
            target=(f" {r.get('target_type')}:{r.get('target_id')}" if r.get('target_id') else "")
            details=f" — {r.get('details')}" if r.get('details') else ""
            lines.append(f"\n• {r.get('created_at')}\n  👤 {html.escape(str(actor))} | {html.escape(str(r.get('action')))}{html.escape(target)}{html.escape(details)}")
        text="".join(lines)
    await callback.message.edit_text(text,parse_mode="HTML",reply_markup=admin_back_button())
    await callback.answer()
...
async def admin_health_callback(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id): return
    lines=["🩺 <b>سلامت سیستم</b>\n", "🤖 Bot: 🟢 فعال", "🗄 Database: 🟢 در دسترس", "🌐 Mini App API: 🟢 متصل"]
    try:
        for p in db.list_vpn_panels():
            if not p.get("enabled"):
                lines.append(f"{p.get('name')}: ⚪ غیرفعال")
                continue
            h=await panel_health_check(p)
            if h["ok"]:
                lines.append(f"{p.get('name')}: 🟢 {h.get('latency_ms')}ms")
            else:
                icon="🔴" if h.get("circuit_open") else "🟠"
                lines.append(f"{p.get('name')}: {icon} {h.get('error') or 'خطا'}")
    except Exception as exc:
        lines.append(f"\n❌ بررسی پنل‌ها: {html.escape(str(exc))}")
    try:
        sentry_state="🟢 فعال" if os.getenv("SENTRY_DSN") else "⚪ تنظیم نشده"
        lines.append(f"Sentry: {sentry_state}")
        pending=len(db.get_pending_online_payments(limit=1000))
        lines.append(f"\n📦 پرداخت آنلاین در انتظار: {pending}")
    except Exception:
        pass
...
async def admin_health_check(message: types.Message, state: FSMContext):
    ...
    try:
        for p in db.list_vpn_panels():
            if not p.get("enabled"):
                lines.append(f"{p.get('name')}: ⚪ غیرفعال"); continue
            h=await panel_health_check(p)
            lines.append(f"{p.get('name')}: {'🟢' if h['ok'] else ('🔴' if h.get('circuit_open') else '🟠')} {h.get('latency_ms') if h['ok'] else (h.get('error') or 'خطا')}{'ms' if h['ok'] else ''}")
    except Exception as exc: lines.append(f"❌ پنل‌ها: {html.escape(str(exc))}")
    try: lines.append(f"Sentry: {'🟢 فعال' if os.getenv('SENTRY_DSN') else '⚪ تنظیم نشده'}")
    except Exception: pass
    await message.answer("\n".join(lines), parse_mode="HTML", reply_markup=_admin_reply_kb_for(message.from_user.id))


# ---------------------------------------------------------------------------
# 🚀 وضعیت کش (سرعت پاسخ‌گویی ربات)
# ---------------------------------------------------------------------------
...
def _cache_status_text() -> str:
    ...
    return (
        "🚀 <b>وضعیت کش ربات</b>\n\n"
        "برای سرعت بیشتر، خیلی از داده‌های پرتکرار (متن‌ها/دکمه‌های ویرایشگر، "
        "پیشوند و تنظیمات اطلاعات ربات، لیست پنل‌های VPN، استیکر هر بخش) به‌جای "
        "خواندن از دیتابیس در هر پیام، از حافظه سرو می‌شوند و فقط وقتی خودشان "
        "عوض بشن دوباره از دیتابیس خونده می‌شن.\n\n"
        f"📦 تعداد آیتم در کش: <b>{st['entries']}</b>\n"
        f"✅ Hit (از حافظه جواب داده شد): <b>{st['hits']}</b>\n"
        f"🔄 Miss (از دیتابیس خونده شد): <b>{st['misses']}</b>\n"
        f"📈 نرخ Hit: <b>{st['hit_rate']}%</b>\n\n"
        "اگه بعد از تغییری در پنل ادمین، جایی هنوز مقدار قدیمی رو می‌بینی "
        "(که طبیعتاً نباید پیش بیاد چون ذخیره‌ها خودشون کش رو آپدیت می‌کنن)، "
        "می‌تونی از پایین کل کش رو دستی پاک کنی."
    )


def _cache_status_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 بروزرسانی آمار", callback_data="admin_cache_status", style="primary")],
        [InlineKeyboardButton(text="🧹 پاک‌کردن کامل کش", callback_data="cache_clear_confirm", style="danger")],
        [InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin_back", style="primary")],
    ])


@router.callback_query(F.data == "admin_cache_status")
async def admin_cache_status_callback(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        return
    await callback.message.edit_text(_cache_status_text(), parse_mode="HTML", reply_markup=_cache_status_keyboard())
    await callback.answer()


@router.message(F.text == "🚀 وضعیت کش")
async def admin_cache_status_message(message: types.Message, state: FSMContext):
    if not _is_admin(message.from_user.id):
        return
    await state.clear()
    await message.answer(_cache_status_text(), parse_mode="HTML", reply_markup=_cache_status_keyboard())


@router.callback_query(F.data == "cache_clear_confirm")
async def admin_cache_clear_confirm(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        return
    await callback.message.edit_text(
        "⚠️ مطمئنی می‌خوای کل کش پاک بشه؟\n\nبعد از پاک شدن، تا چند لحظه‌ی اول ممکنه پاسخ ربات کمی کندتر باشه "
        "(چون دوباره باید از دیتابیس بخونه)، بعدش دوباره سریع می‌شه.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ بله، پاک کن", callback_data="cache_clear_do", style="danger")],
            [InlineKeyboardButton(text="🔙 انصراف", callback_data="admin_cache_status", style="primary")],
        ]),
    )
    await callback.answer()
...
async def admin_cache_clear_do(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id):
        return
    cache.clear_all()
    await callback.message.edit_text(_cache_status_text(), parse_mode="HTML", reply_markup=_cache_status_keyboard())
