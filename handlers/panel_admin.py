...
async def _fulfill_custom_renew(bot, order: dict, order_id: int, volume, days) -> tuple[bool, str]:
    ...
    # بعد از تغییر، مقدار واقعی را دوباره از خود پنل بخوان؛ DB نباید منبع حقیقت باشد.
    snap_ok, snap, _ = await panels.get_service_snapshot(panel, new_service_id or cfg["service_id"])
    snap = snap if snap_ok else {}
    actual_name = snap.get("username") or cfg["plan"].split(" | ")[0]
    actual_expire = snap.get("expire")
    if actual_expire:
        try:
            expiry_date = datetime.fromtimestamp(int(actual_expire)).strftime("%Y-%m-%d")
        except Exception:
    ...
    plan_name = f"{actual_name} | {volume_text} | {days_text}"
...
async def _deliver_panel_link(bot, ctx: dict, link: str):
    ...
    if panel_id and service_id:
        ...
        if panel_obj:
            ...
            if ok_live and isinstance(live, dict):
                snap = {**snap, **live}
    name = snap.get("name") or snap.get("username") or "کاربر"
    ...
    expiry_date = None
    if snap.get("expire"):
        try: expiry_date = datetime.fromtimestamp(int(snap["expire"])).strftime("%Y-%m-%d")
...
async def panel_revoke_sub(callback: types.CallbackQuery):
    ...
    await callback.answer("⏳ در حال ساخت لینک جدید از خود پنل...")
    ok,link,new_id,data,msg=await panels.regenerate_sub_link(panel,cfg["service_id"])
    if not ok or not link:
        await callback.message.answer(f"❌ تغییر لینک در پنل ناموفق بود: {msg}"); return
    db.update_config(cfg_id,cfg["plan"],crypto.encrypt_config(link),expiry=cfg.get("expiry"),service_id=new_id or cfg["service_id"],panel_id=cfg["panel_id"])
    db.set_config_link_disabled(cfg_id,False)
    await callback.message.answer("✅ لینک سرویس از خود پنل تغییر کرد و لینک قبلی باطل شد.",reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="🔙 مشاهده سرویس",callback_data=f"svcdetail_{cfg_id}")]]))
